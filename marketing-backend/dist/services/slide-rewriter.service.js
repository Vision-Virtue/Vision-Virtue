...
exports.rewriteAffectedParagraphs = rewriteAffectedParagraphs;
...
// ─── Prompt construction ─────────────────────────────────────────────────────
const SYSTEM_PROMPT = 'You are an editor fixing partially-filled presentation slides. Each paragraph you ' +
    ...
    'and end with "}".';
function buildUserPrompt(slides) {
    const blocks = slides.map((s) => {
        ...
        const items = s.affected.map((p) => `  { "id": "${p.id}", "text": ${JSON.stringify(p.currentText)} }`).join(',\n');
...
function parseRewriteJson(raw) {
    ...
    try {
        ...
        if (start < 0)
            throw new Error('Rewrite response had no JSON object.');
        const end = findMatchingBrace(candidate, start);
        if (end < 0)
            throw new Error('Rewrite response had an unterminated JSON object.');
        obj = JSON.parse(candidate.slice(start, end + 1));
    }
    const list = (obj && typeof obj === 'object' && 'rewrites' in obj
        ? obj.rewrites
        : null);
    const out = new Map();
 ...
 */
async function rewriteAffectedParagraphs(slides) {
