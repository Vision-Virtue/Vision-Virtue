...
exports.parseGLBuffer = parseGLBuffer;
exports.parseSBBuffer = parseSBBuffer;
...
async function parseXlsx(buf) {
    ...
    for (const r of rawRows) {
        ...
        if (firstDataRow === -1) {
            firstDataRow = rowNum;
            if (/^(gl\s*(number|#|no\.?)|number|account|code)$/i.test(aText))
                continue;
        }
...
function parseCsv(buf) {
    ...
    for (const r of lines) {
        ...
        if (!firstSeen) {
            firstSeen = true;
            if (/^(gl\s*(number|#|no\.?)|number|account|code)$/i.test(a))
                continue;
        }
        if (!a || !b) {
            // Either missing → validation error
            throw new ParseError(`Row "${r.join(',')}" is missing GL Number or GL Name. Both columns are required.`);
        }
...
async function parseGLBuffer(buf, filenameHint) {
    let rows;
    if (isXlsx(buf)) {
        try {
            ...
            throw new ParseError(err instanceof Error
...
function readSBHeaders(a) {
    return /^(company|company\s*name)$/i.test(a);
}
async function parseSBXlsx(buf) {
    const zip = await jszip_1.default.loadAsync(buf);
    const sstFile = zip.file('xl/sharedStrings.xml');
    const sst = [];
    if (sstFile) {
    ...
    for (const r of rawRows) {
        ...
        if (!aText && !bText && !cText && !dText && !eText)
            continue;
        if (firstDataRow === -1) {
            firstDataRow = rowNum;
            if (readSBHeaders(aText))
                continue;
        }
        if (!aText)
            throw new ParseError(`Row ${rowNum}: Company Name (column A) is required.`);
        if (!bText)
            throw new ParseError(`Row ${rowNum}: Employee Name (column B) is required.`);
        if (!dText)
            throw new ParseError(`Row ${rowNum}: Department (column D) is required.`);
        const cost = Number(cText.replace(/[,$\s€£₪]/g, ''));
        const fx = Number(eText.replace(/[,$\s€£₪]/g, ''));
        if (!Number.isFinite(cost))
            throw new ParseError(`Row ${rowNum}: Employer's Cost (column C) must be a number; got "${cText}".`);
        if (!Number.isFinite(fx) || fx <= 0)
            throw new ParseError(`Row ${rowNum}: Exchange rate (column E) must be a positive number; got "${eText}".`);
        result.push({
            companyName: aText,
            employeeName: bText,
            employersCost: cost,
...
function parseSBCsv(buf) {
    ...
    for (const r of lines) {
        ...
        if (!a)
            throw new ParseError(`Row ${lineNo}: Company Name (column A) is required.`);
        if (!b)
            throw new ParseError(`Row ${lineNo}: Employee Name (column B) is required.`);
        if (!d)
            throw new ParseError(`Row ${lineNo}: Department (column D) is required.`);
        const cost = Number(c.replace(/[,$\s€£₪]/g, ''));
        const fx = Number(e.replace(/[,$\s€£₪]/g, ''));
        if (!Number.isFinite(cost))
            throw new ParseError(`Row ${lineNo}: Employer's Cost (column C) must be a number; got "${c}".`);
        if (!Number.isFinite(fx) || fx <= 0)
            throw new ParseError(`Row ${lineNo}: Exchange rate (column E) must be a positive number; got "${e}".`);
        out.push({
            companyName: a,
            employeeName: b,
            employersCost: cost,
...
async function parseSBBuffer(buf, filenameHint) {
    let rows;
    if (isXlsx(buf)) {
        try {
