...
exports.extractPlaceholdersFromXlsx = extractPlaceholdersFromXlsx;
exports.readWorkbookAsText = readWorkbookAsText;
exports.extractPlaceholdersWithClaude = extractPlaceholdersWithClaude;
exports.generatePopulatedPptx = generatePopulatedPptx;
exports.customerPptxDir = customerPptxDir;
exports.pptxTemplatePath = pptxTemplatePath;
exports.buildPptxFileName = buildPptxFileName;
exports.resolveStoredPptx = resolveStoredPptx;
exports.storeUploadedPptx = storeUploadedPptx;
...
const slide_rewriter_service_1 = require("./slide-rewriter.service");
const deck_upload_service_1 = require("./deck-upload.service");
const chart_renderer_service_1 = require("./chart-renderer.service");
...
const EXTRACTOR_MAX_WORKBOOK_CHARS = 35000;
const EXTRACTOR_MAX_UPLOADS_CHARS = 25000;
 ...
 */
function extractPlaceholdersFromSheetXml(sheetXml, sharedStrings, valueColLetter) {
    const out = new Map();
 ...
 */
async function extractPlaceholdersFromXlsx(xlsxPath) {
    ...
    for (const [sheetName, valueCol] of [
        [CALCULATIONS_SHEET, CALCULATIONS_VALUE_COL],
        [QUESTIONNAIRE_SHEET, QUESTIONNAIRE_VALUE_COL],
    ]) {
...
const WORKBOOK_DUMP_SKIP = new Set([
    'Benchmark_Table',
]);
 ...
 */
function readWorkbookAsText(xlsxPath) {
    ...
    for (const name of WORKBOOK_DUMP_PRIORITY) {
        ...
        if (hit && !ordered.includes(hit) && !WORKBOOK_DUMP_SKIP.has(hit))
            ordered.push(hit);
    }
    for (const name of wb.SheetNames) {
        if (WORKBOOK_DUMP_SKIP.has(name))
            continue;
    ...
    for (const sheetName of ordered) {
        ...
        if (csv.length > 40000) {
            csv = csv.slice(0, 40000) +
                `\n\n[... ${csv.length - 40000} chars truncated from sheet "${sheetName}" ...]`;
        }
...
async function collectUniquePlaceholders(templatePptxPath) {
    const buf = await fs_1.default.promises.readFile(templatePptxPath);
    const zip = await jszip_1.default.loadAsync(buf);
    const found = new Set();
    const slideEntries = Object.keys(zip.files).filter((k) => /^ppt\/slides\/slide\d+\.xml$/.test(k));
...
async function gatherUploadsText(customerKeyId) {
    let uploads;
    try {
        uploads = (0, deck_upload_service_1.listUploads)(customerKeyId);
    }
    catch (err) {
...
const EXTRACTOR_SYSTEM_PROMPT = 'You are filling placeholders in a Vision & Virtue investor-deck PowerPoint ' +
...
function clipForBudget(s, maxChars) {
...
function buildExtractorUserPrompt(workbookText, uploadsText, placeholders) {
    ...
    let sources = '';
    if (workbookClipped)
        sources += `EXCEL (all sheets):\n\n${workbookClipped}`;
    if (uploadsClipped)
        sources += (sources ? '\n\n=================\n\n' : '') +
            `CUSTOMER UPLOADS (extracted text):\n\n${uploadsClipped}`;
    if (!sources)
        sources = '(no source data supplied)';
    return `${sources}\n\n=================\n\nPLACEHOLDERS (return a value for every one; "" if unsupported):\n${placeholderBlock}\n\nReturn the JSON object now.`;
}
let _extractorClient = null;
function extractorClient() {
    if (_extractorClient)
        return _extractorClient;
    const key = process.env.ANTHROPIC_API_KEY;
    if (!key)
        throw new Error('ANTHROPIC_API_KEY is not set.');
    _extractorClient = new sdk_1.default({ apiKey: key });
    return _extractorClient;
}
...
async function extractPlaceholdersWithClaude(xlsxPath, templatePptxPath, uploadsText = '') {
    const workbookText = readWorkbookAsText(xlsxPath);
    const placeholders = await collectUniquePlaceholders(templatePptxPath);
    if (placeholders.length === 0)
        return { values: new Map(), charts: new Map() };
    ...
    if (!resp) {
        throw new Error(`Anthropic rate limit (30,000 input tokens/min) hit ${MAX_ATTEMPTS} times in a row ` +
    ...
    for (const [k, v] of Object.entries(parsed)) {
        ...
        if (k.startsWith('{{CHART_') && v && typeof v === 'object') {
            const chart = (0, chart_renderer_service_1.asChartData)(v);
            if (chart) {
                charts.set(k, chart);
                continue;
            }
 ...
 */
function applyReplacementsToSlideXml(slideXml, valueMap, stats) {
    // ── Pass 1: simple intra-run replacements ─────────────────────────────────
    // Replace any {{X}} that sits entirely inside a single <a:t>.
    let xml = slideXml.replace(/<a:t([^>]*)>([\s\S]*?)<\/a:t>/g, (_m, attrs, text) => {
    ...
    xml = xml.replace(/<a:p\b[^>]*>([\s\S]*?)<\/a:p>/g, (paraMatch, paraInner) => {
        ...
        if (runs.length < 2)
            return paraMatch; // single run already handled by pass 1
        ...
        const newRun = `<a:r>${runs[0].rPr}<a:t>${encodeXmlEntities(replaced)}</a:t></a:r>`;
 ...
 */
function cleanupSlideXml(slideXml, originalCount, opts = {}) {
    let stats = {
    ...
    xml = xml.replace(/<a:r\b[^>]*>([\s\S]*?)<\/a:r>/g, (match, inner) => {
    ...
    xml = xml.replace(/<p:txBody\b([^>]*)>([\s\S]*?)<\/p:txBody>/g, (_full, attrs, body) => {
        const processed = body.replace(/<a:p\b[^>]*>([\s\S]*?)<\/a:p>/g, (paraMatch, inner) => {
            const hasContent = /<a:r\b/.test(inner) ||
                /<a:fld\b/.test(inner) ||
...
function countOriginalPlaceholders(xml) {
    const matches = xml.match(/\{\{[A-Z0-9_]+\}\}/g);
 ...
 */
function extractSlideContent(slideXml) {
    let title = '';
    const shapeRe = /<p:sp\b[^>]*>([\s\S]*?)<\/p:sp>/g;
    let sm;
    while ((sm = shapeRe.exec(slideXml))) {
        const inner = sm[1];
        if (!/<p:ph\b[^/>]*type="(?:title|ctrTitle)"/.test(inner))
            continue;
        const tRe = /<a:t\b[^>]*>([\s\S]*?)<\/a:t>/g;
        let parts = '';
        let t;
        while ((t = tRe.exec(inner)))
            parts += decodeXmlEntities(t[1]);
 ...
 */
function replaceParagraphText(slideXml, paraIdx, newText) {
    // Capture the opening <a:p ...> attributes so we don't lose them on emit.
    const paraRe = /<a:p\b([^>]*)>([\s\S]*?)<\/a:p>/g;
    let i = 0;
    return slideXml.replace(paraRe, (match, openAttrs, inner) => {
        const myIdx = i++;
        if (myIdx !== paraIdx)
            return match;
 ...
 */
async function readPresentationSlideOrder(zip) {
 ...
 */
async function renumberSlideFooters(zip, originalTotal) {
    ...
    const pattern = new RegExp(`<a:t([^>]*)>(\\s*)(\\d{1,3})(\\s*/\\s*)${totalEsc}(\\s*)</a:t>`, 'g');
    for (let i = 0; i < order.length; i++) {
        ...
        let touched = false;
        xml = xml.replace(pattern, (_full, attrs, lead, oldIdx, sep, trail) => {
            touched = true;
 ...
 */
function findShapeCoordsContaining(slideXml, placeholder) {
    const shapeRe = /<p:sp\b[^>]*>([\s\S]*?)<\/p:sp>/g;
    let m;
 ...
 */
function findChartHostFrame(slideXml, placeholder) {
    ...
    while ((m = shapeRe.exec(slideXml))) {
        ...
        if (c.x === textShape.x && c.y === textShape.y &&
            c.cx === textShape.cx && c.cy === textShape.cy)
            continue;
 ...
 */
async function ensurePngDefault(zip) {
    ...
    if (/<Default\b[^>]*Extension="png"/i.test(ct))
        return;
    ct = ct.replace('<Types ', '<Types ').replace(/(<Types\b[^>]*>)/, '$1<Default Extension="png" ContentType="image/png"/>');
 ...
 */
async function addImageRelToSlide(zip, slideKey, imageFileName) {
...
function buildPictureShapeXml(picId, picName, rId, coords) {
    return (`<p:pic>` +
        `<p:nvPicPr>` +
 ...
 */
async function injectCharts(zip, chartMap, chartLocations, deletedSlides) {
    if (chartMap.size === 0)
        return 0;
    ...
    for (const [placeholder, chartData] of chartMap) {
        ...
        try {
            ...
            if (aspect > 5)
                heightPx = Math.max(heightPx, Math.round(widthPx / 5));
            else if (aspect < 0.2)
                widthPx = Math.max(widthPx, Math.round(heightPx * 0.2));
            pngBuf = (0, chart_renderer_service_1.renderChartPng)(chartData, widthPx, heightPx);
        }
        catch (err) {
            console.warn(`[pptx-gen] chart ${placeholder} render failed:`, err instanceof Error ? err.message : err);
            continue;
        }
...
function concatVisibleText(xml) {
    const matches = xml.match(/<a:t[^>]*>([\s\S]*?)<\/a:t>/g) || [];
    return matches.map(m => m.replace(/<\/?a:t[^>]*>/g, '')).join('');
}
/** Like findShapeCoordsContaining but tolerant of split text runs. */
function findShapeCoordsFuzzy(slideXml, placeholder) {
    const shapeRe = /<p:sp\b[^>]*>([\s\S]*?)<\/p:sp>/g;
    let m;
    while ((m = shapeRe.exec(slideXml))) {
        const inner = m[1];
        if (!concatVisibleText(inner).includes(placeholder))
            continue;
        const xfrmMatch = /<a:xfrm\b[^>]*>([\s\S]*?)<\/a:xfrm>/.exec(inner);
        if (!xfrmMatch)
            continue;
 ...
 */
function findSplitCompanyLogoCoords(slideXml) {
    const shapeRe = /<p:sp\b[^>]*>([\s\S]*?)<\/p:sp>/g;
    const fragments = [];
    let m;
    while ((m = shapeRe.exec(slideXml))) {
...
function findCustomerLogo(customerKeyId) {
    if (!customerKeyId)
        return null;
    try {
        ...
        if (!img)
            return null;
        const abs = (0, deck_upload_service_1.resolveUpload)(customerKeyId, img.id);
        if (!abs)
            return null;
        ...
        if (!allowed.includes(ext))
            return null;
        return { absPath: abs, extension: ext, mime: img.mimeType };
    }
    catch {
        return null;
    }
...
async function ensureImageDefault(zip, ext) {
    const ctFile = zip.file('[Content_Types].xml');
    if (!ctFile)
        return;
    let ct = await ctFile.async('string');
    const escExt = ext.replace(/[.*+?^${}()|[\]\\]/g, '\\$&');
    if (new RegExp('<Default\\b[^>]*Extension="' + escExt + '"', 'i').test(ct))
        return;
    const mime = IMG_CT[ext] || 'application/octet-stream';
    ct = ct.replace(/(<Types\b[^>]*>)/, '$1<Default Extension="' + ext + '" ContentType="' + mime + '"/>');
    zip.file('[Content_Types].xml', ct);
}
...
async function injectLogo(zip, logo, location, deletedSlides, picId) {
    if (deletedSlides.includes(location.slideKey))
        return false;
    ...
    let imgBuf;
    try {
        imgBuf = fs_1.default.readFileSync(logo.absPath);
    }
    catch {
        return false;
    }
    ...
    slideXml = slideXml.replace(/<p:sp\b[^>]*>([\s\S]*?)<\/p:sp>/g, (whole, inner) => {
        ...
        if (!isHost)
            return whole;
        const stripped = inner.replace(/<a:t[^>]*>[\s\S]*?<\/a:t>/g, (run) => {
 ...
 */
async function deleteSlideFromPackage(zip, slidePath) {
    // 1. Find which rId in presentation rels points at this slide.
    const presRelsFile = zip.file('ppt/_rels/presentation.xml.rels');
    const presFile = zip.file('ppt/presentation.xml');
    const ctFile = zip.file('[Content_Types].xml');
    if (!presRelsFile || !presFile || !ctFile)
        return;
    ...
    if (rid) {
        ...
        presXml = presXml.replace(new RegExp(`<p:sldId\\b[^>]*?\\br:id="${ridEsc}"[^>]*?/>`, 'g'), '');
    ...
    ctXml = ctXml.replace(new RegExp(`<Override\\b[^>]*?\\bPartName="/${slidePathEsc}"[^>]*?/>`, 'g'), '');
    if (notesSlidePath) {
        const notesPathEsc = escapeRegex(notesSlidePath);
        ctXml = ctXml.replace(new RegExp(`<Override\\b[^>]*?\\bPartName="/${notesPathEsc}"[^>]*?/>`, 'g'), '');
    }
    ...
    if (notesSlidePath && zip.file(notesSlidePath))
        zip.remove(notesSlidePath);
    if (notesSlideRelsPath && zip.file(notesSlideRelsPath))
 ...
 */
async function generatePopulatedPptx(opts) {
    if (!fs_1.default.existsSync(opts.templatePptxPath)) {
        throw new Error(`PPTX template not found at ${opts.templatePptxPath}`);
    }
    if (!fs_1.default.existsSync(opts.customerXlsxPath)) {
    ...
    try {
        const result = await extractPlaceholdersWithClaude(opts.customerXlsxPath, opts.templatePptxPath, uploadsText);
    ...
    const pptxBuf = await fs_1.default.promises.readFile(opts.templatePptxPath);
    const zip = await jszip_1.default.loadAsync(pptxBuf);
    const stats = {
    ...
    const finalSlidePath = originalOrder.length > 0 ? originalOrder[originalOrder.length - 1] : null;
    ...
    for (const key of slideEntries) {
        ...
        for (const chartPlaceholder of chartMap.keys()) {
            if (chartLocations.has(chartPlaceholder))
                continue;
            if (!xml.includes(chartPlaceholder))
                continue;
            const coords = findChartHostFrame(xml, chartPlaceholder);
            if (coords)
                chartLocations.set(chartPlaceholder, { slideKey: key, coords });
        }
    ...
    const slidesForRewrite = slideStates
    ...
    if (!sparseDeleteEnabled) {
        console.warn(`[pptx-gen] preserving all template slides (extractionFailed=${!!extractionFailureReason}, ` +
        ...
        if (extractionFailureReason)
            stats.extractionFailureReason = extractionFailureReason;
    }
    for (const slide of slideStates) {
        const { xml: cleaned, stats: slideStats } = cleanupSlideXml(slide.afterReplace, slide.originalCount, {
        ...
        const validation = fast_xml_parser_1.XMLValidator.validate(cleaned);
        if (validation !== true) {
    ...
    if (chartMap.size > 0 && chartLocations.size > 0) {
        try {
            ...
            console.warn('[pptx-gen] chart injection failed (continuing):', err instanceof Error ? err.message : err);
        }
    }
    ...
    if (originalTotal > 0 && (stats.slidesDeleted || 0) > 0) {
        try {
            ...
            console.warn('[pptx-gen] slide renumber failed (continuing):', err instanceof Error ? err.message : err);
        }
    }
    ...
    await fs_1.default.promises.mkdir(path_1.default.dirname(opts.outputPptxPath), { recursive: true });
    await fs_1.default.promises.writeFile(opts.outputPptxPath, outBuf);
    ...
    try {
        ...
        console.warn('[pptx-gen] post-build logo embed failed (deck unaffected):', err instanceof Error ? err.message : err);
    }
    return stats;
}
...
async function injectCompanyLogoPostBuild(opts) {
    ...
    if (!logo) {
        if (opts.customerKeyId)
            console.log('[pptx-gen] post-build: no image upload found, skip logo embed');
        return;
    }
    // 1. Find the placeholder coords from the template.
    const tplBuf = await fs_1.default.promises.readFile(opts.templatePptxPath);
    const tplZip = await jszip_1.default.loadAsync(tplBuf);
    ...
    const outBuf = await fs_1.default.promises.readFile(opts.outputPptxPath);
    const outZip = await jszip_1.default.loadAsync(outBuf);
    ...
    await fs_1.default.promises.writeFile(opts.outputPptxPath, finalBuf);
...
function customerPptxDir() {
    const dbPath = process.env.DB_PATH || './data/marketing.db';
    return path_1.default.join(path_1.default.dirname(path_1.default.resolve(dbPath)), 'customer-pptx');
}
function pptxTemplatePath() {
    return path_1.default.resolve(process.cwd(), 'templates', 'VisionVirtue_InvestorDeck.pptx');
}
/** Filename convention: <safeCustomerName>__<submissionId>.pptx */
function buildPptxFileName(customerName, submissionId) {
    const safe = customerName.replace(/[^a-zA-Z0-9._-]+/g, '_').slice(0, 60) || 'customer';
    return `${safe}__${submissionId}.pptx`;
}
function resolveStoredPptx(fileName) {
    if (!fileName)
        return null;
    const candidate = path_1.default.isAbsolute(fileName)
        ? fileName
        : path_1.default.join(customerPptxDir(), fileName);
 ...
 */
function storeUploadedPptx(submissionId, customerName, buffer) {
    if (buffer.length < 4 ||
