...
class ContentController {
    ...
    async createTopic(req, res) {
        ...
        const item = repository_1.contentRepository.create(topic.trim());
    ...
    async generateEconomistBrief(req, res) {
        const { id } = req.params;
        ...
        let updatedItem = repository_1.contentRepository.update(id, { economist_brief: aiResponse.economist_brief });
        // Only transition if in IDEA_IDENTIFIED state
        if (item.state === 'IDEA_IDENTIFIED') {
            updatedItem = workflow_1.workflowStateMachine.transition(updatedItem, 'ECONOMIST_BRIEF_READY', 'Dr. Ethan Ross (AI)', { model_version: aiResponse.model_version });
        }
        repository_1.contentRepository.addAuditEntry({
            content_id: id,
    ...
    async createMarketingDraft(req, res) {
        const { id } = req.params;
        const item = repository_1.contentRepository.findById(id);
        if (!item) {
            res.status(404).json({ error: { code: 'NOT_FOUND', message: `Content item ${id} not found` } });
        ...
        let updatedItem = repository_1.contentRepository.update(id, { marketing_draft: aiResponse.marketing_draft });
        // Transition to DRAFT_READY if coming from ECONOMIST_BRIEF_READY or RETURNED_FOR_REVISION
        if (item.state === 'ECONOMIST_BRIEF_READY') {
            updatedItem = workflow_1.workflowStateMachine.transition(updatedItem, 'DRAFT_READY', 'Sofia Chen (AI)', { model_version: aiResponse.model_version });
        }
            ...
            updatedItem = repository_1.contentRepository.update(id, { metadata });
        }
        repository_1.contentRepository.addAuditEntry({
            content_id: id,
    ...
    async requestVpReview(req, res) {
        ...
        let updatedItem = repository_1.contentRepository.update(id, { vp_review: aiResponse.vp_review });
        // Transition to UNDER_VP_REVIEW only if not already there (recovery mode skips this)
        if (item.state === 'DRAFT_READY') {
            updatedItem = workflow_1.workflowStateMachine.transition(updatedItem, 'UNDER_VP_REVIEW', 'Daniel Berg (AI)', { decision: aiResponse.vp_review.decision });
        }
        ...
        if (vpDecision === 'APPROVED') {
            updatedItem = workflow_1.workflowStateMachine.transition(updatedItem, 'AWAITING_RAPHAEL_APPROVAL', 'Daniel Berg (AI)', { decision: 'APPROVED', scores: {
            ...
            updatedItem = workflow_1.workflowStateMachine.transition(updatedItem, 'REJECTED', 'Daniel Berg (AI)', { comments: aiResponse.vp_review.comments });
        }
        else {
            // REVISE or any unexpected value — always send for revision rather than getting stuck
            updatedItem = workflow_1.workflowStateMachine.transition(updatedItem, 'RETURNED_FOR_REVISION', 'Daniel Berg (AI)', { comments: aiResponse.vp_review.comments });
        }
        repository_1.contentRepository.addAuditEntry({
            content_id: id,
    ...
    async vpSelfEdit(req, res) {
        ...
        updatedItem = workflow_1.workflowStateMachine.transition(updatedItem, 'DRAFT_READY', 'Daniel Berg (AI)', { self_edit: true });
        updatedItem = workflow_1.workflowStateMachine.transition(updatedItem, 'UNDER_VP_REVIEW', 'Daniel Berg (AI)', { self_edit: true });
        updatedItem = workflow_1.workflowStateMachine.transition(updatedItem, 'AWAITING_RAPHAEL_APPROVAL', 'Daniel Berg (AI)', { self_edit: true, note: 'VP self-edited after 3 revision cycles' });
        repository_1.contentRepository.addAuditEntry({
            content_id: id,
    ...
    async requestApproval(req, res) {
        ...
        const updatedItem = workflow_1.workflowStateMachine.transition(item, 'AWAITING_RAPHAEL_APPROVAL', 'system', { triggered_by: 'manual_request' });
        res.json({ success: true, data: updatedItem });
    }
    ...
    async approve(req, res) {
        ...
        let updatedItem = repository_1.contentRepository.update(id, { approval });
        updatedItem = workflow_1.workflowStateMachine.transition(updatedItem, 'APPROVED_FOR_PUBLISHING', 'Raphael', { notes });
        res.json({ success: true, data: updatedItem });
    }
    ...
    async reject(req, res) {
        ...
        let updatedItem = repository_1.contentRepository.update(id, { approval });
        updatedItem = workflow_1.workflowStateMachine.transition(updatedItem, 'REJECTED', 'Raphael', { notes });
        res.json({ success: true, data: updatedItem });
    }
    ...
    async getById(req, res) {
        ...
        const history = repository_1.contentRepository.getHistory(id);
        res.json({ success: true, data: { ...item, audit_history: history } });
    }
    // GET /api/content
    async getAll(_req, res) {
        const items = repository_1.contentRepository.findAll();
        res.json({ success: true, data: items, count: items.length });
    }
    // POST /api/content/:id/ask-economist
    async askEconomist(req, res) {
        const { id } = req.params;
        ...
        const updatedItem = repository_1.contentRepository.appendQAEntry(id, entry);
        res.json({ success: true, data: updatedItem });
    }
    // POST /api/content/:id/return-to-vp (Raphael sends annotated posts back to Daniel)
    async returnToVp(req, res) {
        const { id } = req.params;
        ...
        let updatedItem = repository_1.contentRepository.update(id, { metadata });
        updatedItem = workflow_1.workflowStateMachine.transition(updatedItem, 'RETURNED_TO_VP_FOR_CORRECTIONS', 'Raphael', { annotation_count: annotations.length });
        repository_1.contentRepository.addAuditEntry({
            content_id: id,
    ...
    async vpCorrect(req, res) {
        ...
        let updatedItem = repository_1.contentRepository.update(id, {
            marketing_draft: aiResponse.marketing_draft,
            metadata,
        });
        // Transition back to Raphael's review
        updatedItem = workflow_1.workflowStateMachine.transition(updatedItem, 'AWAITING_RAPHAEL_APPROVAL', 'Daniel Berg (AI)', { corrections_applied: annotations.length });
        repository_1.contentRepository.addAuditEntry({
            content_id: id,
