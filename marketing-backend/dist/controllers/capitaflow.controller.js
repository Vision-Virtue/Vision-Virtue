"use strict";
/* ============================================================
   CapitaFlow Customer Area — Controller
...
exports.capitaflowController = void 0;
const path_1 = __importDefault(require("path"));
const capitaflow_repository_1 = require("../db/capitaflow.repository");
const marketplace_deck_service_1 = require("../services/marketplace-deck.service");
const nda_pdf_generator_service_1 = require("../services/nda-pdf-generator.service");
const marketplace_extractor_service_1 = require("../services/marketplace-extractor.service");
const deck_token_service_1 = require("../services/deck-token.service");
const system_check_service_1 = require("../services/system-check.service");
const capitaflow_xlsx_service_1 = require("../services/capitaflow-xlsx.service");
const deck_asset_service_1 = require("../services/deck-asset.service");
const deck_upload_service_1 = require("../services/deck-upload.service");
const pptx_generator_service_1 = require("../services/pptx-generator.service");
...
let _aiServiceSingleton = null;
function getAIService() {
    if (_aiServiceSingleton)
        return _aiServiceSingleton;
    const apiKey = process.env.ANTHROPIC_API_KEY;
    if (!apiKey)
        throw new Error('ANTHROPIC_API_KEY is not configured on the server.');
    _aiServiceSingleton = new ai_service_1.AIService(new sdk_1.default({ apiKey }));
    return _aiServiceSingleton;
}
...
const AuthBody = zod_1.z.object({
    ...
    offering: zod_1.z.enum(['visibility', 'capitaflow']).optional(),
});
...
function resolveCustomerKey(req) {
    ...
    if (!headerKey)
        return null;
    return capitaflow_repository_1.customerKeyRepo.findByKey(headerKey.trim(), 'capitaflow');
}
/** Same shape as resolveCustomerKey, but for the investor key (X-Investor-Key). */
function resolveInvestorKey(req) {
    const headerKey = req.header('x-investor-key');
    if (!headerKey)
        return null;
    return capitaflow_repository_1.investorKeyRepo.findByKey(headerKey.trim());
}
...
function send401Investor(res, message) {
    res.status(401).json({
...
function publicListing(listing, baseUrl) {
...
exports.capitaflowController = {
    ...
    auth(req, res) {
        ...
        const row = capitaflow_repository_1.customerKeyRepo.findByKey(parsed.data.key.trim(), parsed.data.offering);
        if (!row) {
    ...
    async createSubmission(req, res) {
        ...
        const sub = capitaflow_repository_1.capitaflowSubmissionRepo.create({
        ...
        try {
            const result = await (0, capitaflow_xlsx_service_1.generateFinalizedXlsx)(sub.id, sub.customerName, sub.formData, keyRow.id);
            capitaflow_repository_1.capitaflowSubmissionRepo.setXlsxPath(sub.id, result.fileName);
            ...
            try {
                ...
                console.error('[capitaflow] pptx generation on submit failed:', pptxErr);
            }
        }
        catch (err) {
            console.error('[capitaflow] xlsx generation on submit failed:', err);
        }
        res.status(201).json({
            id: sub.id,
            status: sub.status,
    ...
    deckSchema(_req, res) {
        res.json({
            sections: investor_deck_schema_1.SECTIONS,
            fields: investor_deck_schema_1.INVESTOR_DECK_FIELDS,
            total: investor_deck_schema_1.INVESTOR_DECK_FIELDS.length,
        });
    },
    /**
    ...
    deckAssetUpload(req, res) {
        ...
        try {
            const stored = (0, deck_asset_service_1.storeAsset)({
                customerKeyId: keyRow.id,
    ...
    deckAssetServe(req, res) {
        ...
        const abs = (0, deck_asset_service_1.resolveAsset)(keyRow.id, fileName);
        if (!abs) {
            res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Asset not found.' } });
            return;
        }
        res.setHeader('Content-Type', (0, deck_asset_service_1.contentTypeForFilename)(fileName));
    ...
    deckUploadCreate(req, res) {
        ...
        try {
            const meta = (0, deck_upload_service_1.storeUpload)({
                customerKeyId: keyRow.id,
                originalName,
     ...
     */
    deckUploadList(req, res) {
        const keyRow = resolveCustomerKey(req);
        if (!keyRow) {
            send401(res, 'Missing or invalid customer key.');
            return;
        }
        res.json({ uploads: (0, deck_upload_service_1.listUploads)(keyRow.id) });
    },
    /**
     ...
     */
    deckUploadDownload(req, res) {
        const keyRow = resolveCustomerKey(req);
        if (!keyRow) {
            send401(res, 'Missing or invalid customer key.');
            return;
        }
        const abs = (0, deck_upload_service_1.resolveUpload)(keyRow.id, req.params.fileId);
        if (!abs) {
            res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Upload not found.' } });
            return;
        }
        const meta = (0, deck_upload_service_1.readMeta)(keyRow.id, req.params.fileId);
        const downloadName = meta?.originalName || 'download';
     ...
     * Header: X-Customer-Key
     */
    deckUploadDelete(req, res) {
        const keyRow = resolveCustomerKey(req);
        if (!keyRow) {
            send401(res, 'Missing or invalid customer key.');
            return;
        }
        const ok = (0, deck_upload_service_1.deleteUpload)(keyRow.id, req.params.fileId);
        if (!ok) {
            res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Upload not found.' } });
            return;
        }
     ...
     */
    adminListUploads(req, res) {
        const sub = capitaflow_repository_1.capitaflowSubmissionRepo.getById(req.params.id);
        if (!sub) {
            res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Submission not found.' } });
            return;
        }
        res.json({ uploads: (0, deck_upload_service_1.listUploads)(sub.customerKeyId) });
    },
    /**
     * GET /api/admin/submissions/:id/uploads/:fileId
     * Streams one customer upload to the admin.
     */
    adminDownloadUpload(req, res) {
        const sub = capitaflow_repository_1.capitaflowSubmissionRepo.getById(req.params.id);
        if (!sub) {
            res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Submission not found.' } });
            return;
        }
        const abs = (0, deck_upload_service_1.resolveUpload)(sub.customerKeyId, req.params.fileId);
        if (!abs) {
            res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Upload not found.' } });
            return;
        }
        const meta = (0, deck_upload_service_1.readMeta)(sub.customerKeyId, req.params.fileId);
        const downloadName = meta?.originalName || 'download';
        res.setHeader('Content-Type', meta?.mimeType || 'application/octet-stream');
        res.setHeader('Content-Disposition', `attachment; filename="${String(downloadName).replace(/"/g, '')}"`);
        res.sendFile(abs);
     ...
     */
    async adminExtractUpload(req, res) {
        const sub = capitaflow_repository_1.capitaflowSubmissionRepo.getById(req.params.id);
        if (!sub) {
            res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Submission not found.' } });
            return;
        ...
        try {
            const text = await (0, deck_upload_service_1.getOrExtractText)(sub.customerKeyId, req.params.fileId);
            res.json({
                ok: true,
                chars: text.length,
    ...
    deckValidation(req, res) {
        ...
        const missingImages = [];
        let filled = 0;
        for (const f of investor_deck_schema_1.INVESTOR_DECK_FIELDS) {
    ...
    listMySubmissions(req, res) {
        ...
        const subs = capitaflow_repository_1.capitaflowSubmissionRepo.listByCustomerKeyId(keyRow.id);
        ...
        const safe = subs.map(s => {
            const pptxOnDisk = !!(0, pptx_generator_service_1.resolveStoredPptx)((0, pptx_generator_service_1.buildPptxFileName)(s.customerName, s.id));
            return {
                id: s.id,
    ...
    downloadMyXlsx(req, res) {
        ...
        const abs = (0, capitaflow_xlsx_service_1.resolveStoredXlsx)(sub.finalizedXlsxPath);
        if (!abs) {
    ...
    adminListSubmissions(_req, res) {
        const subs = capitaflow_repository_1.capitaflowSubmissionRepo.listAll();
            ...
            submissions: subs.map(s => {
                ...
                if (s.customerKeyId) {
                    if (keyCache.has(s.customerKeyId)) {
                        ...
                        keyRow = capitaflow_repository_1.customerKeyRepo.findById(s.customerKeyId);
                ...
                return {
                    ...
                    hasPptx: !!(0, pptx_generator_service_1.resolveStoredPptx)((0, pptx_generator_service_1.buildPptxFileName)(s.customerName, s.id)),
                    formData: s.formData,
                };
            }),
        });
    ...
    async adminFinalize(req, res) {
        ...
        if (!sub.finalizedXlsxPath || !(0, capitaflow_xlsx_service_1.resolveStoredXlsx)(sub.finalizedXlsxPath)) {
            try {
                const result = await (0, capitaflow_xlsx_service_1.generateFinalizedXlsx)(sub.id, sub.customerName, sub.formData, sub.customerKeyId ?? undefined);
                capitaflow_repository_1.capitaflowSubmissionRepo.setXlsxPath(sub.id, result.fileName);
            }
            catch (err) {
                console.error('[capitaflow] xlsx regeneration on finalize failed:', err);
                res.status(500).json({
                    error: {
        ...
        const updated = capitaflow_repository_1.capitaflowSubmissionRepo.markFinalized(sub.id, null);
        res.json({
            submission: {
                id: updated?.id,
    ...
    async adminGenerateXlsx(req, res) {
        ...
        try {
            const result = await (0, capitaflow_xlsx_service_1.generateFinalizedXlsx)(sub.id, sub.customerName, sub.formData, sub.customerKeyId ?? undefined);
            const updated = capitaflow_repository_1.capitaflowSubmissionRepo.setXlsxPath(sub.id, result.fileName);
            res.json({
                submission: {
                    id: updated?.id,
                    status: updated?.status,
            ...
            console.error('[capitaflow] xlsx generation failed:', err);
            res.status(500).json({
                error: {
                    code: 'XLSX_GENERATION_FAILED',
                    message: err instanceof Error ? err.message : 'xlsx generation failed.',
    ...
    adminUploadPptx(req, res) {
        ...
        try {
            (0, pptx_generator_service_1.storeUploadedPptx)(sub.id, sub.customerName, buf);
            ...
            if (sub.status === 'finalized')
                capitaflow_repository_1.capitaflowSubmissionRepo.unfinalize(sub.id);
            const after = capitaflow_repository_1.capitaflowSubmissionRepo.getById(sub.id);
            res.json({ success: true, submission: { id: after?.id, status: after?.status, hasPptx: true } });
        }
        catch (err) {
            console.error('[capitaflow] pptx upload failed:', err);
            res.status(400).json({
    ...
    adminUnfinalize(req, res) {
        ...
        const updated = capitaflow_repository_1.capitaflowSubmissionRepo.unfinalize(sub.id);
    ...
    adminUploadXlsx(req, res) {
        ...
        try {
            const result = (0, capitaflow_xlsx_service_1.storeUploadedXlsx)(sub.id, sub.customerName, buf);
            capitaflow_repository_1.capitaflowSubmissionRepo.setXlsxPath(sub.id, result.fileName);
            ...
            console.error('[capitaflow] xlsx upload failed:', err);
            res.status(400).json({
                error: {
                    code: 'INVALID_XLSX',
    ...
    adminPendingCount(_req, res) {
        res.json({ pending: capitaflow_repository_1.capitaflowSubmissionRepo.countPending() });
    },
    /**
    ...
    adminCreateKey(req, res) {
        const parsed = zod_1.z.object({
            ...
            offering: zod_1.z.enum(['visibility', 'capitaflow']),
        }).safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({ error: { code: 'BAD_REQUEST', message: 'customerName and offering are required.' } });
            return;
        }
        const row = capitaflow_repository_1.customerKeyRepo.create({
    ...
    adminListKeys(_req, res) {
        const rows = capitaflow_repository_1.customerKeyRepo.list();
        res.json({
            keys: rows.map(r => ({
                id: r.id,
                key: r.key,
     ...
     */
    investorAuth(req, res) {
        const parsed = AuthBody.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({ valid: false, error: { code: 'BAD_REQUEST', message: 'Key is required.' } });
            return;
        }
        const row = capitaflow_repository_1.investorKeyRepo.findByKey(parsed.data.key.trim());
        if (!row) {
            res.status(401).json({ valid: false });
            return;
        }
     ...
     */
    adminCreateInvestorKey(req, res) {
        const parsed = zod_1.z.object({ investorName: zod_1.z.string().trim().min(1).max(200) }).safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({ error: { code: 'BAD_REQUEST', message: 'investorName is required.' } });
            return;
        }
        const row = capitaflow_repository_1.investorKeyRepo.create({ investorName: parsed.data.investorName });
        res.status(201).json({
            id: row.id,
            key: row.key,
            investorName: row.investor_name,
     ...
     */
    adminListInvestorKeys(_req, res) {
        const rows = capitaflow_repository_1.investorKeyRepo.list();
        res.json({
            keys: rows.map(r => ({
                id: r.id,
                key: r.key,
    ...
    adminDeleteInvestorKey(req, res) {
        ...
        capitaflow_repository_1.investorKeyRepo.delete(id);
        res.json({ ok: true });
    },
    ...
    adminDeleteCustomerKey(req, res) {
        const id = String(req.params.id || '').trim();
        if (!id) {
            res.status(400).json({ error: { code: 'BAD_REQUEST', message: 'id is required.' } });
            return;
        }
        if (!capitaflow_repository_1.customerKeyRepo.findById(id)) {
            res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Customer key not found.' } });
            return;
        }
        capitaflow_repository_1.customerKeyRepo.delete(id);
        res.json({ ok: true });
    },
    // ────────────────────────────────────────────────────────────────────────────
     ...
     */
    async customerPublishListing(req, res) {
        const keyRow = resolveCustomerKey(req);
        if (!keyRow) {
            send401(res, 'Missing or invalid customer key.');
            return;
        }
        const Body = zod_1.z.object({ submissionId: zod_1.z.string().trim().min(1) });
        const parsed = Body.safeParse(req.body);
        if (!parsed.success) {
            res.status(400).json({ error: { code: 'BAD_REQUEST', message: 'submissionId is required.' } });
            return;
        }
        const sub = capitaflow_repository_1.capitaflowSubmissionRepo.getById(parsed.data.submissionId);
        if (!sub || sub.customerKeyId !== keyRow.id) {
            res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Submission not found.' } });
            return;
        }
        let extracted;
        try {
            extracted = await (0, marketplace_extractor_service_1.extractMarketplaceTileData)(sub);
        }
        catch (err) {
        ...
        let logoPath = null;
        try {
            const uploads = (0, deck_upload_service_1.listUploads)(keyRow.id);
            const imgUpload = uploads.find(u => u.mimeType && u.mimeType.startsWith('image/'));
            if (imgUpload)
                logoPath = 'marketplace-logo:' + imgUpload.id;
        }
        catch { /* best-effort */ }
        const listing = capitaflow_repository_1.marketplaceListingRepo.upsert({
            submissionId: sub.id,
            customerKeyId: keyRow.id,
            customerName: sub.customerName,
            logoPath,
     ...
     * preview shown in the CapitaFlow area before the customer clicks Publish.
     */
    async customerPreviewListing(req, res) {
        const keyRow = resolveCustomerKey(req);
        if (!keyRow) {
            send401(res, 'Missing or invalid customer key.');
            return;
        ...
        const sub = capitaflow_repository_1.capitaflowSubmissionRepo.getById(submissionId);
        if (!sub || sub.customerKeyId !== keyRow.id) {
            res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Submission not found.' } });
            return;
        }
        try {
            const extract = await (0, marketplace_extractor_service_1.extractMarketplaceTileData)(sub);
            res.json({ extract });
        }
        catch (err) {
            const msg = err instanceof Error ? err.message : 'Auto-extraction failed.';
     ...
     * Header: X-Customer-Key
     * Returns the customer's own listings (active + withdrawn) so the CapitaFlow
     * area can show current status + offer Pull submission.
     */
    customerListMyListings(req, res) {
        const keyRow = resolveCustomerKey(req);
        if (!keyRow) {
            send401(res, 'Missing or invalid customer key.');
            return;
        }
        const listings = capitaflow_repository_1.marketplaceListingRepo.listByCustomerKey(keyRow.id);
        res.json({ listings });
    },
    /**
     ...
     */
    customerWithdrawListing(req, res) {
        const keyRow = resolveCustomerKey(req);
        if (!keyRow) {
            send401(res, 'Missing or invalid customer key.');
            return;
        ...
        const id = String(req.params.id || '').trim();
        const listing = capitaflow_repository_1.marketplaceListingRepo.findById(id);
        if (!listing || listing.customerKeyId !== keyRow.id) {
            res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Listing not found.' } });
            return;
        }
        const updated = capitaflow_repository_1.marketplaceListingRepo.withdraw(id);
     ...
     */
    investorListListings(req, res) {
        const keyRow = resolveInvestorKey(req);
        if (!keyRow) {
            send401Investor(res, 'Missing or invalid investor key.');
            return;
        }
        const baseUrl = backendBaseUrl(req);
        const listings = capitaflow_repository_1.marketplaceListingRepo.listActive().map(l => publicListing(l, baseUrl));
        res.json({ listings });
    },
    /**
    ...
    investorGetListing(req, res) {
        ...
        const nda = capitaflow_repository_1.ndaSignatureRepo.findByInvestorAndListing(keyRow.id, listing.id);
        ...
        const sub = capitaflow_repository_1.capitaflowSubmissionRepo.getById(listing.submissionId);
        if (sub) {
            pptxOnDisk = !!(0, pptx_generator_service_1.resolveStoredPptx)((0, pptx_generator_service_1.buildPptxFileName)(sub.customerName, sub.id));
        }
        res.json({
    ...
    customerUploadDeckPdf(req, res) {
        ...
        if (body.length > marketplace_deck_service_1.MAX_DECK_PDF_BYTES) {
            res.status(413).json({ error: { code: 'PDF_TOO_LARGE', message: 'PDF exceeds 30 MB.' } });
            return;
        }
        try {
            const ref = (0, marketplace_deck_service_1.storeDeckPdf)(listing.id, body);
            const updated = capitaflow_repository_1.marketplaceListingRepo.setDeckPdfPath(listing.id, ref);
    ...
    investorSignNda(req, res) {
        ...
        const Body = zod_1.z.object({
            listingId: zod_1.z.string().trim().min(1),
            fullName: zod_1.z.string().trim().min(2).max(200),
            fundName: zod_1.z.string().trim().min(1).max(200),
            title: zod_1.z.string().trim().min(1).max(120),
            businessEmail: zod_1.z.string().trim().email().max(200),
            signDate: zod_1.z.string().trim().regex(/^\d{4}-\d{2}-\d{2}$/, 'Use YYYY-MM-DD'),
            signatureType: zod_1.z.enum(['typed', 'drawn']),
            signatureValue: zod_1.z.string().trim().min(1).max(500000), // data-URL PNG can be large
            confirmed: zod_1.z.literal(true).optional(), // checkbox state, advisory
        });
        const parsed = Body.safeParse(req.body);
        if (!parsed.success) {
        ...
        const listing = capitaflow_repository_1.marketplaceListingRepo.findById(parsed.data.listingId);
        if (!listing || listing.status !== 'active') {
            res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Listing not available.' } });
            return;
        }
        const ip = (req.ip || req.headers['x-forwarded-for'] || '').toString().slice(0, 60);
        const ua = (req.headers['user-agent'] || '').toString().slice(0, 400);
        const nda = capitaflow_repository_1.ndaSignatureRepo.create({
    ...
    investorNdaStatus(req, res) {
        ...
        if (!listingId) {
            res.status(400).json({ error: { code: 'BAD_REQUEST', message: 'listingId required.' } });
            return;
        }
        const nda = capitaflow_repository_1.ndaSignatureRepo.findByInvestorAndListing(keyRow.id, listingId);
        res.json({
            signed: !!nda,
    ...
    investorGetDeckInfo(req, res) {
        ...
        const pptxAbsPath = (0, pptx_generator_service_1.resolveStoredPptx)((0, pptx_generator_service_1.buildPptxFileName)(sub.customerName, sub.id));
        if (!pptxAbsPath) {
            res.status(404).json({
                error: {
                    code: 'DECK_MISSING',
        ...
        const token = (0, deck_token_service_1.signDeckToken)({ listingId: listing.id, investorKeyId: keyRow.id });
     ...
     * TTL the URL is dead.
     */
    marketplaceServeDeckPptx(req, res) {
        const token = String(req.params.token || '').trim();
        const claims = (0, deck_token_service_1.verifyDeckToken)(token);
        if (!claims) {
            res.status(401).json({ error: { code: 'INVALID_TOKEN', message: 'Link expired — return to the marketplace and click the tile again.' } });
            return;
        }
        const listing = capitaflow_repository_1.marketplaceListingRepo.findById(claims.listingId);
        if (!listing || listing.status !== 'active') {
            res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Listing not available.' } });
            return;
        }
    ...
    marketplaceServeLogo(req, res) {
        ...
        const abs = (0, deck_upload_service_1.resolveUpload)(listing.customerKeyId, fileId);
        if (!abs) {
            res.status(404).end();
            return;
        }
        const meta = (0, deck_upload_service_1.readMeta)(listing.customerKeyId, fileId);
    ...
    adminSystemCheck(_req, res) {
        res.json((0, system_check_service_1.runSystemCheck)());
    },
    // ────────────────────────────────────────────────────────────────────────────
    ...
    adminListNdaSignatures(_req, res) {
        const rows = capitaflow_repository_1.ndaSignatureRepo.listAll();
        res.json({
    ...
    async adminDownloadNdaPdf(req, res) {
        const id = String(req.params.id || '').trim();
        const nda = capitaflow_repository_1.ndaSignatureRepo.findById(id);
        if (!nda) {
            res.status(404).json({ error: { code: 'NOT_FOUND', message: 'NDA not found.' } });
            return;
        }
        try {
            const buf = await (0, nda_pdf_generator_service_1.generatePopulatedNdaPdf)(nda);
    ...
    adminDeleteNda(req, res) {
        ...
        capitaflow_repository_1.ndaSignatureRepo.delete(id);
        res.json({ ok: true });
    },
    // ────────────────────────────────────────────────────────────────────────────
     ...
     * template.
     */
    async adminGeneratePptx(req, res) {
        const sub = capitaflow_repository_1.capitaflowSubmissionRepo.getById(req.params.id);
        if (!sub) {
            res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Submission not found.' } });
            return;
        ...
        const xlsxAbs = (0, capitaflow_xlsx_service_1.resolveStoredXlsx)(sub.finalizedXlsxPath);
        if (!xlsxAbs) {
        ...
        try {
            ...
            console.error('[capitaflow] adminGeneratePptx failed:', err);
            res.status(500).json({
                error: {
     ...
     */
    adminDownloadPptx(req, res) {
        const sub = capitaflow_repository_1.capitaflowSubmissionRepo.getById(req.params.id);
        if (!sub) {
            res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Submission not found.' } });
            return;
        }
        const fileName = (0, pptx_generator_service_1.buildPptxFileName)(sub.customerName, sub.id);
        const abs = (0, pptx_generator_service_1.resolveStoredPptx)(fileName);
        if (!abs) {
            res.status(409).json({
    ...
    updateMySubmission(req, res) {
        ...
        const updated = capitaflow_repository_1.capitaflowSubmissionRepo.updateFormData(sub.id, parsed.data.formData);
    ...
    async adminChat(req, res) {
        ...
        const { message, history = [], submissionId } = (req.body || {});
        ...
        const allSubs = capitaflow_repository_1.capitaflowSubmissionRepo.listAll();
        ...
        if (submissionId) {
            ...
            const formDataDump = (() => {
                try {
                    return JSON.stringify(sub.formData, null, 2);
                }
                catch {
            ...
            if (sub.finalizedXlsxPath) {
                ...
                if (abs && fs_1.default.existsSync(abs)) {
                    try {
                        workbookDump = (0, pptx_generator_service_1.readWorkbookAsText)(abs).slice(0, 60000);
                    }
                    catch (err) {
                        workbookDump = `[Workbook read failed: ${err.message}]`;
                    }
                }
            }
            focusBlock = [
     ...
     */
    async consultEthan(req, res) {
        const keyRow = resolveCustomerKey(req);
        if (!keyRow) {
            send401(res, 'Missing or invalid customer key.');
            return;
        }
        const { message, history = [], agent = 'vc_expert' } = (req.body || {});
        if (!message || typeof message !== 'string' || !message.trim()) {
            res.status(400).json({ error: { code: 'BAD_REQUEST', message: 'message is required.' } });
            return;
        }
 ...
 */
async function generateDeckForSubmission(submissionId, customerName, xlsxAbsPath, customerKeyId) {
    const outDir = (0, pptx_generator_service_1.customerPptxDir)();
    if (!fs_1.default.existsSync(outDir))
        fs_1.default.mkdirSync(outDir, { recursive: true });
    const outFile = `${outDir}/${(0, pptx_generator_service_1.buildPptxFileName)(customerName, submissionId)}`.replace(/\\/g, '/');
    ...
    const pptxStats = await (0, pptx_generator_service_1.generatePopulatedPptx)({
        templatePptxPath: (0, pptx_generator_service_1.pptxTemplatePath)(),
