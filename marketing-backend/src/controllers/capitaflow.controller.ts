...
import {
  customerKeyRepo, capitaflowSubmissionRepo, CustomerKeyRow,
...
import { generateFinalizedXlsx, resolveStoredXlsx, storeUploadedXlsx } from '../services/capitaflow-xlsx.service';
import {
...
export const capitaflowController = {
  /**
  ...
  async createSubmission(req: Request, res: Response): Promise<void> {
    ...
    const sub = capitaflowSubmissionRepo.create({
    ...
    try {
      ...
      capitaflowSubmissionRepo.setXlsxPath(sub.id, result.fileName);
  ...
  adminListUploads(req: Request, res: Response): void {
    const sub = capitaflowSubmissionRepo.getById(req.params.id);
    if (!sub) {
  ...
  listMySubmissions(req: Request, res: Response): void {
    ...
    const subs = capitaflowSubmissionRepo.listByCustomerKeyId(keyRow.id);
  ...
  adminListSubmissions(_req: Request, res: Response): void {
    const subs = capitaflowSubmissionRepo.listAll();
  ...
  async adminFinalize(req: Request, res: Response): Promise<void> {
    ...
    const updated = capitaflowSubmissionRepo.markFinalized(sub.id, null);
    res.json({
      submission: {
        id:          updated?.id,
  ...
  async adminGenerateXlsx(req: Request, res: Response): Promise<void> {
    ...
    try {
      const result  = await generateFinalizedXlsx(sub.id, sub.customerName, sub.formData as never, sub.customerKeyId ?? undefined);
      const updated = capitaflowSubmissionRepo.setXlsxPath(sub.id, result.fileName);
      res.json({
        submission: {
          id:      updated?.id,
          status:  updated?.status,
  ...
  adminUploadPptx(req: Request, res: Response): void {
    ...
    try {
      ...
      if (sub.status === 'finalized') capitaflowSubmissionRepo.unfinalize(sub.id);
      const after = capitaflowSubmissionRepo.getById(sub.id);
  ...
  adminUnfinalize(req: Request, res: Response): void {
    ...
    const updated = capitaflowSubmissionRepo.unfinalize(sub.id);
  ...
  adminPendingCount(_req: Request, res: Response): void {
    res.json({ pending: capitaflowSubmissionRepo.countPending() });
  },
  ...
  async customerPublishListing(req: Request, res: Response): Promise<void> {
    ...
    const sub = capitaflowSubmissionRepo.getById(parsed.data.submissionId);
    if (!sub || sub.customerKeyId !== keyRow.id) {
      res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Submission not found.' } });
      return;
    }
  ...
  async customerPreviewListing(req: Request, res: Response): Promise<void> {
    ...
    const sub = capitaflowSubmissionRepo.getById(submissionId);
    if (!sub || sub.customerKeyId !== keyRow.id) {
      res.status(404).json({ error: { code: 'NOT_FOUND', message: 'Submission not found.' } });
      return;
    }
  ...
  investorGetListing(req: Request, res: Response): void {
    ...
    const sub = capitaflowSubmissionRepo.getById(listing.submissionId);
    if (sub) {
  ...
  updateMySubmission(req: Request, res: Response): void {
    ...
    const updated = capitaflowSubmissionRepo.updateFormData(sub.id, parsed.data.formData);
  ...
  async adminChat(req: Request, res: Response): Promise<void> {
    ...
    const allSubs = capitaflowSubmissionRepo.listAll();
