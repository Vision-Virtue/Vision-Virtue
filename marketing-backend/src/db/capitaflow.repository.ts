...
export interface CapitaFlowSubmission {
  id: string;
...
function rowToSubmission(row: CapitaFlowSubmissionRow): CapitaFlowSubmission {
  return {
    id: row.id,
    customerKeyId: row.customer_key_id,
    customerName: row.customer_name,
...
export const capitaflowSubmissionRepo = {
  create(input: {
    customerKeyId: string | null;
    customerName: string;
    formData: unknown;
  }): CapitaFlowSubmission {
    const id = uuidv4();
    const submittedAt = new Date().toISOString();
    getDb()
      .prepare(
        `INSERT INTO capitaflow_submissions
    ...
    return this.getById(id) as CapitaFlowSubmission;
  },

  getById(id: string): CapitaFlowSubmission | null {
    const row = getDb()
      .prepare('SELECT * FROM capitaflow_submissions WHERE id = ?')
      .get(id) as CapitaFlowSubmissionRow | undefined;
    return row ? rowToSubmission(row) : null;
  },

  /** Submissions belonging to a specific customer key, newest first. */
  listByCustomerKeyId(customerKeyId: string): CapitaFlowSubmission[] {
    const rows = getDb()
      .prepare(
        'SELECT * FROM capitaflow_submissions WHERE customer_key_id = ? ORDER BY submitted_at DESC',
      )
      .all(customerKeyId) as CapitaFlowSubmissionRow[];
    return rows.map(rowToSubmission);
  },

  /** Every submission, newest first (admin view). */
  listAll(): CapitaFlowSubmission[] {
    const rows = getDb()
      .prepare('SELECT * FROM capitaflow_submissions ORDER BY submitted_at DESC')
      .all() as CapitaFlowSubmissionRow[];
    return rows.map(rowToSubmission);
  },
  ...
  setXlsxPath(id: string, xlsxPath: string): CapitaFlowSubmission | null {
    getDb()
  ...
  markFinalized(id: string, notes?: string | null): CapitaFlowSubmission | null {
    getDb()
      .prepare(
        `UPDATE capitaflow_submissions
  ...
  finalize(id: string, xlsxPath: string, notes?: string | null): CapitaFlowSubmission | null {
    getDb()
      .prepare(
        `UPDATE capitaflow_submissions
   ...
   */
  updateFormData(id: string, formData: unknown): CapitaFlowSubmission | null {
    getDb()
      .prepare(
        `UPDATE capitaflow_submissions
   ...
   */
  unfinalize(id: string): CapitaFlowSubmission | null {
    getDb()
      .prepare(
        `UPDATE capitaflow_submissions
