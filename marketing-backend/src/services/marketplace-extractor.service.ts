...
export interface MarketplaceTileExtract {
  sector: string;
  description: string;
...
function extractDescription(f: Record<string, unknown>): string {
  const candidates = [
...
function extractAsk(f: Record<string, unknown>): string {
  const a = f.askDesc;
  if (typeof a === 'string' && a.trim()) return a.trim().slice(0, 100);
  return '';
}
...
function normalizeKpiValue(v: unknown): string | undefined {
...
export async function extractMarketplaceTileData(
  ...
  const sector       = String((f.sector as string) || '').trim() || 'Other';
  ...
  const xlsxPath = submission.finalizedXlsxPath
    ? resolveStoredXlsx(submission.finalizedXlsxPath)
    : null;
  if (!xlsxPath) {
  ...
  let workbookText = '';
  try { workbookText = readWorkbookAsText(xlsxPath); } catch { workbookText = ''; }
  if (!workbookText) {
    return { sector, description, askAmountText: askFromForm, kpis: {} };
  }

  const prompt =
    `You are reading a populated Vision & Virtue Financial Model workbook for the ` +
