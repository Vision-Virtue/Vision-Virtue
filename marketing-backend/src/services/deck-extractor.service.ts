...
function fillablePlaceholders(): InvestorDeckField[] {
  return INVESTOR_DECK_FIELDS.filter(
    (f) => f.inputType !== 'image' && !CALC_PLACEHOLDERS.has(f.placeholder),
  );
}
...
async function buildSourceText(customerKeyId: string): Promise<string> {
...
function buildSystemPrompt(): string {
  return (
    'You are a senior financial analyst preparing an investor presentation. ' +
...
function buildUserPrompt(sources: string, fields: InvestorDeckField[]): string {
...
function extractJsonMap(raw: string): Record<string, string> {
 ...
 */
function sanitize(raw: Record<string, unknown>, fields: InvestorDeckField[]): Map<string, string> {
...
export interface DeckExtractionResult {
  /** Filled placeholders the AI extracted. */
  values: Map<string, string>;
  /** How many were filled. */
  filledCount: number;
  /** How many we asked the model to consider. */
  candidateCount: number;
  /** Raw Claude response, truncated for logging. */
  rawResponseSnippet: string;
  /** Char count of source materials sent. */
  sourceChars: number;
  /** Model used. */
  model: string;
}
