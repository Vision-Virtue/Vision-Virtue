...
const UPLOADS_TEXT_CAP = 30_000;
const PER_UPLOAD_CAP   = 8_000;

const SHEET_NAME = 'Business Presentation Calcs';

export interface BusinessCalc {
  label: string;
  value: string;
  unit: string;
  source_file: string;
 ...
 */
export async function extractBusinessCalcs(opts: {
  customerKeyId: string;
  workbookSheetNames: string[];
  customerName?: string;
}): Promise<BusinessCalc[]> {
  const uploads = listUploads(opts.customerKeyId);
  if (uploads.length === 0) return [];

  const parts: string[] = [];
  for (const u of uploads) {
    try {
      const text = await getOrExtractText(opts.customerKeyId, u.id);
 ...
 */
export async function listWorkbookSheetNames(xlsxPath: string): Promise<string[]> {
 ...
 */
export async function appendBusinessCalcsSheet(
  xlsxPath: string,
  calcs: BusinessCalc[],
): Promise<void> {
  ...
  if (!wbEntry) throw new Error('xl/workbook.xml not found in xlsx');
  ...
  const existingSheetFiles = Object.keys(zip.files).filter(
    (k) => /^xl\/worksheets\/sheet\d+\.xml$/.test(k),
  );
  ...
  while ((m = nameRe.exec(workbookXml)) !== null) existingNames.add(m[1]);
  if (existingNames.has(chosenName)) {
    let i = 2;
    while (existingNames.has(`${chosenName} ${i}`)) i++;
  ...
  const overrideTag =
    `<Override PartName="/xl/worksheets/sheet${newSheetNum}.xml" ` +
  ...
  await fs.promises.writeFile(xlsxPath, outBuf);
}
...
function buildSheetXml(calcs: BusinessCalc[]): string {
